...
export function ItemDetailDialog({ item, stock, open, onClose }: ItemDetailDialogProps) {
  ...
  const handleConsume = () => {
    ...
    // For executors, service order is required
    if (currentUser.role === 'executor' && !serviceOrder.trim()) {
    ...
    addMovement({
      ...
      reason: serviceOrder ? `Serviço: ${serviceOrder}` : (item.isConsumable ? 'Consumo de item' : 'Uso definitivo'),
  ...
  const handleLoanToUser = () => {
    ...
    addMovement({
      ...
      quantity,
      executorUserId: currentUser.id,
      reason: `Empréstimo para ${responsibleUser?.name}`,
      observations: observations || undefined,
    });
  ...
  const handleRemoveStock = () => {
    ...
    addMovement({
      ...
      quantity: -quantity,
      executorUserId: currentUser.id,
      reason: 'Ajuste de estoque (remoção)',
      observations,
    });
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          {/* Stock Info */}
          {stock && (
            <div className="bg-slate-50 rounded-lg p-3 sm:p-4 space-y-3">
